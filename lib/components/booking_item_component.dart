...
class BookingItemComponentState extends State<BookingItemComponent> {
  ...
  Future<void> _startDriveFromList(BookingData b) async {
    final now = DateTime.now();
    final start = DateFormat(BOOKING_SAVE_FORMAT).format(now);
    final end = b.endAt.validate();
    final diff = b.durationDiff.validate().isEmptyOrNull
        ? '0'
        : b.durationDiff.validate();
    final pay = b.isAdvancePaymentDone
        ? SERVICE_PAYMENT_STATUS_ADVANCE_PAID
        : b.paymentStatus.validate();
    final request = {
      CommonKeys.id: b.id,
      BookingUpdateKeys.startAt: start,
      BookingUpdateKeys.endAt: end,
      BookingUpdateKeys.durationDiff: diff,
      BookingUpdateKeys.reason: '',
      BookingUpdateKeys.status: BookingStatusKeys.onGoing,
      BookingUpdateKeys.paymentStatus: pay,
    };
    appStore.setLoading(true);
    try {
      await bookingUpdate(request);
        ...
        children: [
          ...
          if ((isUserTypeProvider || isUserTypeHandyman) &&
            ...
            Row(
              children: [
                AppButton(
                  child: Text(languages.lblStartDrive,
                      style: boldTextStyle(color: white)),
                  width: context.width(),
                  color: primaryColor,
                  elevation: 0,
                  onTap: () async {
                    await showConfirmDialogCustom(
                      ...
                      negativeText: languages.lblNo,
                      onAccept: (_) async {
                        await _startDriveFromList(widget.bookingData);
                      },
                    );
                  },
                ).expand(),
