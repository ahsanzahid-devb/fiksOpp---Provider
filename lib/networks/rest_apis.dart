  ...
  try {
    ...
    void addQueryPart(String key, String value, {bool encode = false}) {
      ...
      final processedValue =
          encode ? Uri.encodeQueryComponent(value.trim()) : value.trim();
      queryParts.add('$key=$processedValue');
    }

    addQueryPart('service_id', serviceId);
    addQueryPart('date_from', dateFrom);
    addQueryPart('date_to', dateTo);
    addQueryPart('customer_id', customerId);
    addQueryPart('provider_id', providerId);
    addQueryPart('handyman_id', handymanId);

    if (bookingStatus.trim().isNotEmpty &&
        bookingStatus.trim() != BOOKING_PAYMENT_STATUS_ALL) {
      addQueryPart('status', bookingStatus);
    }

    addQueryPart('payment_status', paymentStatus);
    addQueryPart('payment_type', paymentType);
    addQueryPart('search', searchText, encode: true);

    String queryString = queryParts.join('&');

    Future<BookingListResponse> request(String qs) async {
      return BookingListResponse.fromJson(await handleResponse(
          await buildHttpResponse('booking-list?$qs',
              method: HttpMethodType.GET)));
    }

    try {
      res = await request(queryString);
    } catch (e) {
      ...
      if (shouldRetry &&
          (queryString.contains('provider_id=') ||
              queryString.contains('handyman_id='))) {
        final filteredParts = queryParts
            .where((p) =>
                !p.startsWith('provider_id=') && !p.startsWith('handyman_id='))
            .toList();
        final fallbackQuery = filteredParts.join('&');
        res = await request(fallbackQuery);
      } else {
        rethrow;
      }
    }
